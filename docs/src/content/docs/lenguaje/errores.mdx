...
description: ojo, cago y los errores de runtime — cómo WN++ falla y cómo manejarlo.
---

import { Aside, Tabs, TabItem, Card, CardGrid, Badge } from '@astrojs/starlight/components';

En WN++ hay dos categorías de cosas que pueden salir mal: los **errores de runtime** que el lenguaje lanza automáticamente, y los errores que tú capturas con `ojo`/`cago`. Ambos hablan chileno.
...
Cualquier error de runtime puede ser atrapado con `ojo`/`cago`. Estos son todos los que WN++ puede lanzar:
...
Estos ocurren **antes** de que el programa corra, no durante. No se pueden capturar con `ojo`/`cago` porque WN++ ni llega a ejecutar el código.
